...
ebisumart_recovery_schema: spark_types.StructType = spark_types.StructType([
    ...
    spark_types.StructField("FREE_ITEM_11", spark_types.StringType(), True)
])
...
ebisumart_recovery_df: DataFrame = spark.read.format("csv").options(**{
    ...
    "inferSchema": False,
    "multiLine": False
}).schema(ebisumart_recovery_schema).load(azure_storage_path_format.format(os.path.join(jp_ebisumart_inbound_path, "FY*.csv")))
...
demand_recovery_payment_df: DataFrame = header_df
demand_recovery_payment_final_df: DataFrame = demand_recovery_payment_df.alias("payment").join(ebisumart_tender_mapping_df.alias("tender"), how="left", on=[spark_functions.col("payment.Tender_ID") == spark_functions.col("tender.TenderID")]).selectExpr(
    "eCom_Store_ID",
    "Transaction_Nbr",
    "Order_Date",
...
(
    demand_recovery_header_final_df
        .coalesce(1)
        .write
        .format("csv")
        .options(**{
...
(
    demand_recovery_payment_final_df
        .coalesce(1)
        .write
        .format("csv")
        .options(**{
...
demand_recovery_line_final_df: DataFrame = (line_df
                                            .alias("line")
